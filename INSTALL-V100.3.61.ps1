...
if ($CurrentVersion -notin @("100.3.60", "100.3.61")) { throw "Expected V100.3.60 or V100.3.61, found V$CurrentVersion." }
...
npm ci
npm run check
if ($LASTEXITCODE -ne 0) { throw "V100.3.61 validation failed." }
npm run certify:pilot
if ($LASTEXITCODE -ne 0) { throw "V100.3.61 certification failed." }
...
Write-Host ""
Write-Host "V100.3.61 installed and certified." -ForegroundColor Green
Write-Host "Runtime database preserved: $RuntimePath"
Write-Host "Restart Blue Current, sign in, then test Staff Coverage on the iPad."
