...
(async()=>{
 ...
 const stale=snapshot.active.find(item=>item.id==="tc_old"),current=snapshot.active.find(item=>item.id==="tc_current");
 ok("Old open punch remains visible",Boolean(stale));
 ...
 ok("Staff view prioritizes review exceptions",staff.includes("Number(b.requiresReview)-Number(a.requiresReview)"));
