...
const root=path.resolve(__dirname,"../.."),maintenance=__dirname,release="100.3.50";
...
const tests=fs.readdirSync(maintenance).filter(name=>parts(name)).sort((a,b)=>compare(parts(a),parts(b))||a.localeCompare(b)).filter(name=>compare(parts(name),[10,3])>=0&&compare(parts(name),[50])<=0);
...
const completedAt=new Date();console.log("\n=== BLUE CURRENT HOSTED RESTART REHEARSAL ===");console.log(JSON.stringify({release,status:"HOSTED_RESTART_REHEARSAL_READY",automaticDeployment:false,sourceDatabaseMutation:false,dnsChanged:false,startedAt:startedAt.toISOString(),completedAt:completedAt.toISOString(),durationMs:completedAt-startedAt,gatesPassed:results.length,gatesTotal:results.length,results},null,2));
