...
check("Build retains V100.3.75 audit closure", ["100.3.75","100.3.76","100.3.77","100.3.78","100.3.79"].includes(pkg.version) && html.includes('content="100.3.79"'));
...
check("Pointerdown navigation remains inside the same delegated shell owner", /function installPrimaryNavigation\(\)[\s\S]*navigation\.addEventListener\("pointerdown"[\s\S]*commit\(name\)/.test(shell));
...
check("Updated shell, readiness, and style assets cross a fresh cache boundary", html.includes("styles.css?v=100.3.79") && html.includes("hospitalityOsShell.js?v=100.3.79") && html.includes("productionReadiness.js?v=100.3.79"));
