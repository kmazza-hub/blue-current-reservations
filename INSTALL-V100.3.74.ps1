...
if ($CurrentVersion -ne "100.3.73") { throw "Expected certified V100.3.73; found V$CurrentVersion." }
...
$Files = @(
  "client\index.html", "client\styles.css", "client\js\modules\authOrganizations.js",
  ...
  "INSTALL-V100.3.74.ps1", "README-V100.3.74.md"
)
...
npm ci
npm run check
if ($LASTEXITCODE -ne 0) { throw "V100.3.74 validation failed." }
npm run certify:pilot
if ($LASTEXITCODE -ne 0) { throw "V100.3.74 certification failed." }
...
Write-Host ""
Write-Host "V100.3.74 installed and certified." -ForegroundColor Green
