...
ready(()=>{
 ...
 // Connection recovery banner only appears when verified connectivity requires it.
 let banner=null;
 ...
 window.addEventListener("bluecurrent:connectivity-state",event=>{
   ...
   if(state==="checking"){
     if(title)title.textContent="Verifying live connection";
     if(body)body.textContent="Network access returned. Blue Current is confirming the restaurant server before resuming live status.";
     if(button)button.disabled=true;
   }else if(state==="unreachable"){
     if(title)title.textContent="Blue Current is not reachable";
     if(body)body.textContent="The network is available, but the restaurant server has not been verified. Keep the current screen visible and retry.";
     if(button)button.disabled=false;
   }else{
     if(title)title.textContent="Live connection interrupted";
     if(body)body.textContent="Blue Current will keep the current screen visible while the connection recovers.";
     if(button)button.disabled=false;
   }
 });
