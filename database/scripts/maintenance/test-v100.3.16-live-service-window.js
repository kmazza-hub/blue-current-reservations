...
const context={Date:{now:()=>now},Number,JSON,Array,String,localStorage:{getItem:key=>store.get(key)||null,setItem:(key,value)=>store.set(key,value),removeItem:key=>store.delete(key)},window:{dispatchEvent:event=>events.push(event)},CustomEvent:function(name,options){this.type=name;this.detail=options.detail}};
...
ok("Pruned cache is persisted",JSON.parse(store.get("blueCurrent.service.activeParties.v100.loc_marina")).length===1);
...
ok("Floor cache key advances",/floor-reservations-v62\.0\.js\?v=100\.3\.(?:16|17|19)/.test(index));
ok("Kitchen cache key advances",/kitchen-truth-v100\.2\.60\.js\?v=100\.3\.(?:16|19)/.test(index));
ok("Runtime loader cache key advances",/kitchen-truth-v100\.2\.60\.js\?v=100\.3\.(?:16|19)/.test(runtime));
