...
const root=path.resolve(__dirname,"../.."),maintenance=__dirname,release="100.3.44";
const parts=name=>{const m=name.match(/^test-v100\.3\.(\d+(?:\.\d+)*)-/);return m?m[1].split(".").map(Number):null;};
const compareParts=(a,b)=>{for(let i=0;i<Math.max(a.length,b.length);i+=1){const d=(a[i]||0)-(b[i]||0);if(d)return d;}return 0;};
const tests=fs.readdirSync(maintenance).filter(name=>parts(name)).sort((a,b)=>compareParts(parts(a),parts(b))||a.localeCompare(b)).filter(name=>compareParts(parts(name),[10,3])>=0&&compareParts(parts(name),[44])<=0);
