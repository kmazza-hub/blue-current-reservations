...
const root=path.resolve(__dirname,"../.."),maintenance=__dirname,release="100.3.51";
...
const tests=fs.readdirSync(maintenance).filter(name=>parts(name)).sort((a,b)=>compare(parts(a),parts(b))||a.localeCompare(b)).filter(name=>compare(parts(name),[10,3])>=0&&compare(parts(name),[51])<=0);
...
console.log("\n=== BLUE CURRENT CONTROLLED DEMONSTRATION ===");console.log(JSON.stringify({release,status:"CONTROLLED_DEMONSTRATION_READY",fictionalRestaurant:true,pilotEvidenceMutation:false,liveSystemWriteBack:false,physicalIpadCertificationPerformed:false,gatesPassed:results.length,gatesTotal:results.length,durationMs:Date.now()-startedAt},null,2));
