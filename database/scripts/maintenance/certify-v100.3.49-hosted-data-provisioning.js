...
const root=path.resolve(__dirname,"../.."),maintenance=__dirname,release="100.3.49";
...
const tests=fs.readdirSync(maintenance).filter(name=>parts(name)).sort((a,b)=>compare(parts(a),parts(b))||a.localeCompare(b)).filter(name=>compare(parts(name),[10,3])>=0&&compare(parts(name),[49])<=0);
...
const completedAt=new Date();console.log("\n=== BLUE CURRENT HOSTED DATA PROVISIONING ===");console.log(JSON.stringify({release,status:"HOSTED_DATA_PROVISIONING_READY",automaticProvisioning:false,overwriteAllowed:false,automaticDeployment:false,startedAt:startedAt.toISOString(),completedAt:completedAt.toISOString(),durationMs:completedAt-startedAt,gatesPassed:results.length,gatesTotal:results.length,results},null,2));
