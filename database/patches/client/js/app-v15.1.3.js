...
(() => {
  ...
  const getArrival = () => [...document.querySelectorAll("#arrivalQueue .queue-item.arrival")]
    .find((item) => item.querySelector("strong")?.textContent?.trim() === guestName) || null;

  const arrivalStatus = () => {
    const arrival = getArrival();
    if (!arrival) return "expected";
    if (arrival.dataset.bcGuestStatus) return arrival.dataset.bcGuestStatus;
    const text = arrival.querySelector(".arrival-chip")?.textContent?.trim().toLowerCase() || "";
  ...
  window.__bcHostAssignmentV100_2_13 = assignment;
  ...
  const ensureArrivalAction = () => {
    const arrival = getArrival();
    if (!arrival) return null;
    const chip = arrival.querySelector(".arrival-chip");
    if (!chip) return null;

    let action = arrival.querySelector(".bc-arrival-action-v100-2-13");
    if (!action) {
      action = document.createElement("button");
      action.type = "button";
      action.className = "bc-arrival-action-v100-2-13";
      action.textContent = "Mark arrived";
      action.setAttribute("aria-label", `Mark ${guestName} arrived`);
      chip.insertAdjacentElement("afterend", action);
    }

    const status = arrivalStatus();
    action.hidden = status !== "expected";
    return action;
  };

  const updateArrivalState = (status, tableNumber = null) => {
    const arrival = getArrival();
    if (!arrival) return;
    const chip = arrival.querySelector(".arrival-chip");
    arrival.dataset.bcGuestStatus = status;

    if (status === "arrived") {
      arrival.removeAttribute("data-bc-table");
      arrival.setAttribute("aria-label", `${guestName}, arrived`);
      if (chip) {
        chip.classList.remove("pending", "bc-seated-table");
        chip.textContent = "Arrived";
      }
    }
    ...
    const action = ensureArrivalAction();
    if (action) action.hidden = status !== "expected";
  };

  const updateButtonForSelection = (table) => {
    const number = tableNumberOf(table);
    const state = getTableState(table);
    const guestStatus = arrivalStatus();
    button.hidden = false;
    button.disabled = false;
    delete button.dataset.bcTargetTable;

    if (assignment.status === "seated" || guestStatus === "seated") {
      button.hidden = true;
      button.disabled = true;
      setLifecycleStatus(`${guestName} is already seated at Table ${assignment.tableNumber || table.dataset.bcTable || number}.`);
      return;
    }

    if (assignment.status === "assigned" && assignment.table === table) {
      button.dataset.bcTargetTable = number;
      if (guestStatus === "arrived") {
        button.dataset.bcAssignmentState = "assigned";
        button.disabled = false;
        button.textContent = `Seat ${shortName} at Table ${number}`;
        setLifecycleStatus(`${guestName} has arrived. Table ${number} is held and ready to seat.`);
      } else {
        button.dataset.bcAssignmentState = "held";
        button.disabled = true;
        button.textContent = `Table ${number} held for ${shortName}`;
        setLifecycleStatus(`Table ${number} is held for ${guestName}. Mark the reservation arrived before seating.`);
      }
      return;
    }
    ...
    if (guestStatus === "arrived") {
      button.dataset.bcAssignmentState = "seat-ready";
      button.textContent = `Seat ${shortName} at Table ${number}`;
      setLifecycleStatus(`${guestName} has arrived. Table ${number} is ${state} and ready for one-tap seating.`);
    } else {
      button.dataset.bcAssignmentState = "ready";
      button.textContent = `Reserve Table ${number} for ${shortName}`;
      setLifecycleStatus(`${guestName} is expected. Hold Table ${number} now, then seat after arrival.`);
    }
  };
  ...
  const selectTable = (table) => {
    ...
    table.classList.add("selected");
    window.__bcHostSelectedTableV100_2_13 = table;
  ...
  const reserveTableForGuest = (table) => {
    ...
    setSelectedHeading(table);
    setGuestGrid(table);
    $("#hostRecommendation") && ($("#hostRecommendation").textContent = `Table ${number} held for ${guestName} at 7:30 PM`);
    updateButtonForSelection(table);
  };
  ...
  const seatAssignedGuest = () => {
    ...
    updateArrivalState("seated", number);
  ...
  const seatArrivedGuestDirectly = (table) => {
    reserveTableForGuest(table);
    seatAssignedGuest();
  };

  if (!window.__bcDynamicTableSelectionV100_2_13) {
    window.__bcDynamicTableSelectionV100_2_13 = true;
    tables().forEach((table) => table.addEventListener("click", () => selectTable(table)));
  }

  const arrivalAction = ensureArrivalAction();
  if (arrivalAction && !arrivalAction.dataset.bcBoundV100213) {
    arrivalAction.dataset.bcBoundV100213 = "true";
    arrivalAction.addEventListener("click", () => {
      updateArrivalState("arrived");
      const selected = window.__bcHostSelectedTableV100_2_13;
      if (selected) updateButtonForSelection(selected);
    });
  }

  if (!window.__bcDynamicAssignmentClickV100_2_13) {
    window.__bcDynamicAssignmentClickV100_2_13 = true;
    button.addEventListener("click", () => {
      const state = button.dataset.bcAssignmentState || "blocked";
      const selected = window.__bcHostSelectedTableV100_2_13;
      if (!selected || state === "blocked" || state === "held" || button.disabled) return;
      ...
      if (state === "seat-ready") {
        if (!isAssignable(selected)) return updateButtonForSelection(selected);
        seatArrivedGuestDirectly(selected);
        return;
      }
      ...
      if (state === "assigned") {
        ...
        if (arrivalStatus() !== "arrived") return updateButtonForSelection(selected);
        seatAssignedGuest();
      }
    });
  }

  // V100.2.13 keeps arrival state visible throughout the host flow and starts
  // with no transient table-detail card until the host explicitly selects a table.
...
(() => {
  ...
  const partySizeFrom = (detail) => Number(String(detail || '').match(/party\s+of\s+(\d+)/i)?.[1] || 1);
  ...
  const complete = () => {
    ...
    const num = tableNumber(table);
    const prior = tableState(table);
    const partySize = Math.max(1, Number(flow.partySize || 1));
    ...
    const small = table.querySelector('small');
    if (small) small.textContent = 'Seated';
    window.__bcHostTableTrustV100_2_22?.markSeated?.(table, partySize, guestName);

    syncGuestSeated(guestName, num);
    window.dispatchEvent(new CustomEvent('bc:host-guest-seated',{detail:{guest:guestName,partySize,guestDetail:flow.guestDetail,source:flow.row?.dataset?.bcSourceType||'waitlist',tableId:num,seatedAt}}));
...
const platform = window.BlueCurrentPlatform.create({ build: "44.12.0-aip-multi-agent-orchestration", eventBus });
...
const selectedCandidatePreviewCenterModule = startupRegistry.register("selectedCandidatePreviewCenter", shouldInitializeCenter("selectedCandidatePreviewCenter") ? window.createBlueCurrentSelectedCandidatePreviewCenterModule?.(eventBus, appState) : null, ["eventBus", "appState", "retirementCandidateSelectionCenter"]);
const v46ReleaseCertificationCenterModule = startupRegistry.register("v46ReleaseCertificationCenter", shouldInitializeCenter("v46ReleaseCertificationCenter") ? window.createBlueCurrentV46ReleaseCertificationCenterModule?.(eventBus, appState) : null, ["eventBus", "appState", "selectedCandidatePreviewCenter"]);
...
const restaurantDayLifecycleModule = startupRegistry.register("restaurantDayLifecycle", shouldInitializeCenter("restaurantDayLifecycle") ? window.createBlueCurrentRestaurantDayLifecycleCenterModule?.(eventBus, appState) : null, ["eventBus", "appState", "pilotOperationalReadiness"]);
...
const v54OperatorExperienceCertificationV5500Module = startupRegistry.register("v54OperatorExperienceCertificationV5500", shouldInitializeCenter("v54OperatorExperienceCertificationV5500") ? window.createBlueCurrentV54OperatorExperienceCertificationCenterModule?.(eventBus, appState) : null, ["eventBus", "appState", "roleBasedServiceErgonomicsV5475"]);
const restaurantIntelligenceDecisionSupportV5525Module = startupRegistry.register("restaurantIntelligenceDecisionSupportV5525", shouldInitializeCenter("restaurantIntelligenceDecisionSupportV5525") ? window.createBlueCurrentRestaurantIntelligenceDecisionSupportCenterModule?.(eventBus, appState) : null, ["eventBus", "appState", "v54OperatorExperienceCertificationV5500"]);
const profitabilityInterventionAccountabilityV5575Module = startupRegistry.register("profitabilityInterventionAccountabilityV5575", shouldInitializeCenter("profitabilityInterventionAccountabilityV5575") ? window.createBlueCurrentProfitabilityInterventionAccountabilityCenterModule?.(eventBus, appState) : null, ["eventBus", "appState", "restaurantIntelligenceDecisionSupportV5525"]);
const v55DecisionValueCertificationV5600Module = startupRegistry.register("v55DecisionValueCertificationV5600", shouldInitializeCenter("v55DecisionValueCertificationV5600") ? window.createBlueCurrentV55DecisionValueCertificationCenterModule?.(eventBus, appState) : null, ["eventBus", "appState", "profitabilityInterventionAccountabilityV5575"]);
...
const finalHardeningRealEnvironmentV5850Module = startupRegistry.register("finalHardeningRealEnvironmentV5850", shouldInitializeCenter("finalHardeningRealEnvironmentV5850") ? window.createBlueCurrentFinalHardeningRealEnvironmentCenterModule?.(eventBus, appState) : null, ["eventBus", "appState", "finalProductReleaseCandidateV5800"]);
const productionLaunchCertificationV5900Module = startupRegistry.register("productionLaunchCertificationV5900", shouldInitializeCenter("productionLaunchCertificationV5900") ? window.createBlueCurrentProductionLaunchCertificationCenterModule?.(eventBus, appState) : null, ["eventBus", "appState", "finalHardeningRealEnvironmentV5850"]);
...
const pilotSessionCenterModule = startupRegistry.register("pilotSessionCenter",shouldInitializeCenter("pilotSessionCenter") ? window.createBlueCurrentPilotSessionCenterModule?.(eventBus, appState) : null,["eventBus","appState","enterprisePilotCutoverCenter","locationCutoverCenter"]);
...
const executiveKpiStudioCenterModule = startupRegistry.register("executiveKpiStudioCenter",shouldInitializeCenter("executiveKpiStudioCenter") ? window.createBlueCurrentExecutiveKpiStudioCenterModule?.(eventBus, appState) : null,["eventBus","appState","executiveDecisionWorkspaceV43Center"]);
const executiveTimelineCenterV43Module = startupRegistry.register("executiveTimelineCenterV43",shouldInitializeCenter("executiveTimelineCenterV43") ? window.createBlueCurrentExecutiveTimelineCenterV43Module?.(eventBus, appState) : null,["eventBus","appState","executiveInsightCenter","executiveRecommendationCenterV43"]);
const executivePortfolioHealthCenterModule = startupRegistry.register("executivePortfolioHealthCenter",shouldInitializeCenter("executivePortfolioHealthCenter") ? window.createBlueCurrentExecutivePortfolioHealthCenterModule?.(eventBus, appState) : null,["eventBus","appState","executiveKpiStudioCenter","executiveTimelineCenterV43"]);
const executiveKnowledgeGraphCenterModule = startupRegistry.register("executiveKnowledgeGraphCenter",shouldInitializeCenter("executiveKnowledgeGraphCenter") ? window.createBlueCurrentExecutiveKnowledgeGraphCenterModule?.(eventBus, appState) : null,["eventBus","appState","executivePortfolioHealthCenter"]);
...
const aipMultiAgentOrchestrationCenterModule = startupRegistry.register("aipMultiAgentOrchestrationCenter",shouldInitializeCenter("aipMultiAgentOrchestrationCenter") ? window.createBlueCurrentAipMultiAgentOrchestrationCenterModule?.(eventBus, appState) : null,["eventBus","appState","aipPersistentAgentRuntimeCenter","aipRuntimeLifecycleCenter"]);
const aipApprovalOrchestrationCenterModule = startupRegistry.register("aipApprovalOrchestrationCenter",shouldInitializeCenter("aipApprovalOrchestrationCenter") ? window.createBlueCurrentAipApprovalOrchestrationCenterModule?.(eventBus, appState) : null,["eventBus","appState","aipMultiAgentOrchestrationCenter"]);
const aipDurableWorkflowCenterModule = startupRegistry.register("aipDurableWorkflowCenter",shouldInitializeCenter("aipDurableWorkflowCenter") ? window.createBlueCurrentAipDurableWorkflowCenterModule?.(eventBus, appState) : null,["eventBus","appState","aipMultiAgentOrchestrationCenter","aipApprovalOrchestrationCenter"]);
const aipWorkflowSupervisorCenterModule = startupRegistry.register("aipWorkflowSupervisorCenter",shouldInitializeCenter("aipWorkflowSupervisorCenter") ? window.createBlueCurrentAipWorkflowSupervisorCenterModule?.(eventBus, appState) : null,["eventBus","appState","aipDurableWorkflowCenter"]);
...
(() => {
  const detail = document.getElementById('hostTableDetail');
  const floorMap = document.querySelector('.host-floor-map');
  if (!detail || !floorMap) return;
  if (detail.dataset.bcDismissV100233 === 'true') return;
  detail.dataset.bcDismissV100233 = 'true';

  const closeNeutralDetail = () => {
    detail.classList.remove('bc-detail-open');
    detail.hidden = true;
    detail.setAttribute('aria-hidden', 'true');
    floorMap.querySelectorAll('.host-table.selected').forEach((table) => table.classList.remove('selected'));
  };

  let closeButton = detail.querySelector('.bc-neutral-detail-close-v100-2-33');
  if (!closeButton) {
    closeButton = document.createElement('button');
    closeButton.type = 'button';
    closeButton.className = 'bc-neutral-detail-close-v100-2-33';
    closeButton.setAttribute('aria-label', 'Close table details');
  ...
  floorMap.addEventListener('click', (event) => {
    if (event.target.closest('.host-table, #hostTableDetail')) return;
    if (!detail.hidden && detail.classList.contains('bc-neutral-table-detail-v100-2-16')) closeNeutralDetail();
  });
  ...
  document.querySelectorAll('.host-side-nav button, .host-top-nav button, [data-host-workspace]').forEach((control) => {
...
(() => {
  const map = document.getElementById('hostFloorMap');
  if (!map || map.dataset.bcPremiumFloorV100235 === 'true') return;
  map.dataset.bcPremiumFloorV100235 = 'true';

  const architecture = document.createElement('div');
  architecture.className = 'bc-floor-architecture-v100-2-35';
