(function () {
  ...
  function createBlueCurrentGuidedShiftCenterModule(eventBus, appState) {
    ...
    const setText = (id, value) => { const node = byId(id); if (node) node.textContent = value; return node; };
    const setHtml = (id, value) => { const node = byId(id); if (node) node.innerHTML = value; return node; };
    const esc = value => String(value ?? "").replace(/[&<>"']/g, char => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[char]));
    let filter = "open";

    function render(snapshot = engine.snapshot()) {
      setText("guidedShiftUpdated",  `Updated ${new Date(snapshot.capturedAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}`);
      setText("guidedShiftPhase",  snapshot.phase.label);
      setText("guidedShiftPhaseNote",  snapshot.phase.note);
      setText("guidedShiftUnacknowledged",  snapshot.unacknowledged);
      setText("guidedShiftOpenHandoffs",  snapshot.openHandoffs.length);
      setText("guidedShiftHandoffNote",  snapshot.openHandoffs.length ? `${snapshot.openHandoffs.length} handoff${snapshot.openHandoffs.length === 1 ? "" : "s"} still open.` : "No handoff blocking the shift.");
      setText("guidedShiftFocusScore",  snapshot.focusScore);
      setText("guidedShiftStatus",  snapshot.unacknowledged ? `${snapshot.unacknowledged} event${snapshot.unacknowledged === 1 ? "" : "s"} need acknowledgement` : "Operator queue clear");
      [["Now", snapshot.plan.now], ["Next", snapshot.plan.next], ["Later", snapshot.plan.later]].forEach(([name, item]) => {
        setText(`guidedShift${name}Title`, item.title);
        setText(`guidedShift${name}Detail`, item.detail);
        setText(`guidedShift${name}Impact`, item.impact);
      });
    ...
    byId("guidedShiftCreateHandoff")?.addEventListener("click", () => {
      try {
        const snapshot = engine.createHandoff({ owner: byId("guidedShiftHandoffOwner")?.value || "Next manager", note: byId("guidedShiftHandoffNoteInput")?.value || "" });
        if (byId("guidedShiftHandoffNoteInput")) byId("guidedShiftHandoffNoteInput").value = "";
        setText("guidedShiftActionStatus", "Handoff created.");
        render(snapshot);
      } catch (error) {
        setText("guidedShiftActionStatus", error.message);
      }
      setTimeout(() => setText("guidedShiftActionStatus", ""), 4000);
    });
