(() => {
  ...
  const STORAGE_KEY = "blueCurrent.managerActions.v34.0.5a";

  const sampleActions = [
    {
      id: "approve-pto",
      title: "Review Sarah’s pending PTO request",
      source: "Workforce",
      priority: "high",
      due: "Due today",
      completed: false
    ...
    {
      id: "wing-order",
      title: "Confirm supplemental wing order",
      source: "Inventory",
      priority: "high",
      due: "Before 10:00 AM",
      completed: false
    ...
    {
      id: "ice-machine",
      title: "Inspect ice machine before lunch",
      source: "Maintenance",
      priority: "medium",
      due: "Due 10:30 AM",
      completed: false
    ...
    {
      id: "produce-delivery",
      title: "Confirm produce delivery arrival",
      source: "Operations",
      priority: "medium",
      due: "Expected 9:00–10:00 AM",
      completed: false
    },
    {
      id: "guest-review",
      title: "Reply to overnight guest review",
      source: "Guests",
      priority: "low",
      due: "Due today",
      completed: true
  ...
  function loadActions() {
    try {
      ...
      console.warn("[ActionList] Stored data could not be read.", error);
    }
    return sampleActions.map((action) => ({ ...action }));
  }
  ...
  function saveActions() {
    try {
      ...
      console.warn("[ActionList] Changes could not be saved.", error);
    }
  }
  ...
  function createActionElement(action) {
    ...
    copy.querySelector("small").textContent = `${action.source} · ${action.due}`;
  ...
  function render() {
    ...
    status.textContent = open.length
      ? `${open.length} action${open.length === 1 ? "" : "s"} remaining`
    ...
    if (state.filter === "completed") {
      ...
      list.append(createEmpty(state.filter === "high"
        ? "No open high-priority actions."
