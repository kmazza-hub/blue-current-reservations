...
class BlueCurrentShiftIntelligenceEngine{
 constructor({eventBus,appState}){this.eventBus=eventBus;this.appState=appState;this.off=[];["state:updated","trusted-dataset:updated","predictive-overlay:updated","kitchen-throughput:updated","labor-deployment:updated"].forEach(n=>this.off.push(eventBus.on(n,()=>this.refresh(n))));}
 ...
 calculate(reason="manual"){
  ...
  const occupancy=this.n(s.occupancyPercent,0), kitchen=this.n(s.kitchenThroughput?.load??s.predictiveOverlay?.kitchenLoad??s.kitchenLoad,Math.min(96,occupancy+6));
  const wait=this.n(s.guestWaitMinutes??s.predictiveOverlay?.guestWait,Math.max(3,Math.round((occupancy-60)/2.5)));
  const labor=this.n(s.laborDeployment?.laborPercent??s.marginIntelligence?.laborCostPercent,31);
  const reservations=this.n(s.reservationsToday,0), expected=this.n(s.guestsExpected,0);
  const serviceQuality=this.n(s.serviceQuality?.score,Math.max(45,100-wait*2-Math.max(0,kitchen-82)));
  ...
  const value={reason,capturedAt:new Date().toISOString(),score,status:score>=85?"strong":score>=70?"watch":"attention",occupancy,kitchenLoad:kitchen,guestWait:wait,laborPercent:labor,serviceQuality,reservations,expectedGuests:expected,peakMinutes,recommendations:ranked,topAction:ranked[0],history:[...(previous.history||[]),{at:new Date().toISOString(),score,kitchen,wait}].slice(-80)};
  this.appState.update({shiftIntelligence:value});this.eventBus.emit("shift-intelligence:updated",structuredClone(value));return value;
 }
