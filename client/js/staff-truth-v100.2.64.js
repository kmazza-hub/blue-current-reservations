...
function ensureStyles(){
 ...
 .bc-staff-truth-v264 .bc-staff-tools{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:0 0 18px}.bc-staff-truth-v264 .bc-staff-tools button{min-height:58px;border:1px solid #b8d4da;border-radius:14px;background:#fff;color:#123b45;font-size:16px;font-weight:900;cursor:pointer}.bc-staff-truth-v264 .bc-staff-tools button:hover,.bc-staff-truth-v264 .bc-staff-tools button:focus-visible{background:#e8f8f7;border-color:#42b9b4;outline:3px solid rgba(66,185,180,.25)}
 @media(max-width:760px){.bc-staff-truth-v264{padding:20px 12px}.bc-staff-truth-v264 .bc-staff-head{display:block}.bc-staff-truth-v264 .bc-staff-head button{margin-top:12px}.bc-staff-truth-v264 .bc-staff-tools{grid-template-columns:1fr}.bc-staff-truth-v264 .bc-staff-kpis{grid-template-columns:1fr 1fr}.bc-staff-truth-v264 .bc-staff-priority{display:block}.bc-staff-truth-v264 .bc-staff-priority button{margin-top:12px;width:100%}.bc-staff-truth-v264 .bc-staff-row{grid-template-columns:1fr}}
 `;
...
function openStaffSection(id){
 ...
 try{window.BlueCurrentHospitalityShell?.activate?.("team",{scroll:false});}catch{}
 const target=byId(id);if(!target)return;
 target.hidden=false;target.removeAttribute("aria-hidden");
 requestAnimationFrame(()=>{target.scrollIntoView({behavior:"smooth",block:"start"});target.setAttribute("tabindex","-1");target.focus({preventScroll:true});});
 if(id==="workforce-foundation")setTimeout(()=>byId("wffRefresh")?.click(),80);
 if(id==="scheduling")setTimeout(()=>window.BlueCurrentSchedulingTruthV100_2_73?.refresh?.(),80);
 if(id==="time-clock")setTimeout(()=>window.BlueCurrentTimeClockTruthV100_2_76?.refresh?.(),80);
}
const tools=()=>`<nav class="bc-staff-tools" aria-label="Staff operations"><button type="button" data-bc-staff-section="workforce-foundation">Employees, availability & PTO</button><button type="button" data-bc-staff-section="scheduling">Schedule & publish</button><button type="button" data-bc-staff-section="time-clock">Time clock & corrections</button></nav>`;
function bindTools(view){view.querySelectorAll("[data-bc-staff-section]").forEach(button=>button.addEventListener("click",()=>openStaffSection(button.dataset.bcStaffSection)));}
...
async function load(){
 ...
 try{
   ...
   if(!client?.token){renderUnavailable("Sign in to load the current clocked-in team.");return;}
   const state=await client.timeClock(LOCATION_ID);
   render(state||{});
