(() => {
  ...
  const appSource = `js/app-v15.1.3.js?v=39.12.0`;
  ...
  function selectedPlaceholders() {
    ...
    if (requestedPacks.has("operations")) {
      const required = /(?:recommendationCalibration|operationsLearningReview|nextShiftPlan|decisionExecutionLedger|outcomeCapture|shiftCloseBrief|priorityFocus|actionOwnership|shiftHandoffSnapshot|operationsWorkspace|shiftIntelligence|executiveDecisionFeed|operationsCopilot)(?:Engine|Center)\.js/i;
  ...
  async function boot() {
    try {
      ...
      if (summary) summary.textContent = `V39.12.0 ready · ${duration}ms`;
