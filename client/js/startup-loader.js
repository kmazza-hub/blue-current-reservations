(() => {
  ...
  const appSource = `js/app-v15.1.3.js?v=41.29.0`;
  ...
  function selectedPlaceholders() {
    ...
    if (requestedPacks.has("aip")) {
      const requiredAip = /(?:aipToolRegistry|aipAgentRuntime|aipPromptOrchestrator|aipGovernance|aipMissionControl|aipApprovalQueue|aipContextGraph|aipMemoryVault|aipScenarioLab|aipImprovementBacklog|aipPromptExperiment|aipLearningReleaseGate|aipCommandConsole|aipExecutionPlan|aipOutcomeReview|aipPolicyComposer|aipToolGateway|aipRunScheduler|aipPolicyEnforcement|aipExecutionQueue|aipAutonomyBoundary|hospitalityOntology|decisionObject|causalDecisionTrace|operationalMemory|decisionHorizonForecast|executiveReasoningBrief|portfolioReasoning|decisionLeverage|portfolioCoordinationPlan|adaptiveStrategy|decisionTradeoff|executiveWorkspace)(?:Engine|Center)\.js/i;
  ...
  async function boot() {
    try {
      ...
      if (summary) summary.textContent = `V41.26.0 ready · ${duration}ms`;
