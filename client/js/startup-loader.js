(() => {
  ...
  const appSource = `js/app-v15.1.3.js?v=39.0.0`;
  ...
  function selectedPlaceholders() {
    ...
    // V39.0 — preserve the minimal Operations slice even when older HTML metadata is cached.
    if (requestedPacks.has("operations")) {
      const required = /(?:shiftIntelligence|executiveDecisionFeed|operationsCopilot)(?:Engine|Center)\.js/i;
  ...
  async function boot() {
    try {
      ...
      if (summary) summary.textContent = `V39.0.0 ready · ${duration}ms`;
