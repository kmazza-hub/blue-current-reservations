...
if ($CurrentVersion -ne "100.3.72") { throw "Expected certified V100.3.72; found V$CurrentVersion." }
...
$Files = @(
  ...
  "server\persistence\runtimeBackupManager.js", "package.json", "package-lock.json",
  ...
  "scripts\maintenance\certify-v100.3.73-render-runtime-marker-continuity.js",
  "INSTALL-V100.3.73.ps1", "README-V100.3.73.md"
)
...
npm ci
npm run check
if ($LASTEXITCODE -ne 0) { throw "V100.3.73 validation failed." }
npm run certify:pilot
if ($LASTEXITCODE -ne 0) { throw "V100.3.73 certification failed." }
...
Write-Host "";Write-Host "V100.3.73 installed and certified." -ForegroundColor Green
